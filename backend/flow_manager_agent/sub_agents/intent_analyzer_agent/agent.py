...
BASE_NLU_SPEC = r"""
    ...
    Gratitude / appreciation ONLY (conversation closing), e.g.:
      "תודה", "תודה רבה", "אלוף אתה", "מהמם", "עזרת לי", "thanks","thank you","awesome","great","appreciate it"
      → return:
        {
          "status": "not_relevant",
    ...
    If the user provides ONLY a dimension name (e.g. app_id, media_source, site_id, partner)
    without a value and without an analytical request:

    → clarification_needed
    → missing_fields must include:
      - "metric"
    → The clarification message must ask only:
      "Which metric would you like to analyze regarding <dimension>?"
